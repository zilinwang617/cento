...
export function splitPaper(note, gap, separation = 10) {
  ...
  const angle = note.angle * Math.PI / 180;
  const makeHalf = (text, width, centerOffset, textOffset, side) => {
  ...
  return [
    makeHalf(leftText, gap.x, (gap.x - note.width) / 2, gap.textLeft, -1),
    makeHalf(rightText, note.width - gap.x, gap.x / 2, gap.right - gap.x, 1),
  ];
}
